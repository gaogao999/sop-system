...
- **Home shelves** — when you're not searching, the page shows **🗂 All documents** (every current document, newest first, with a status badge — so anything just registered is always visible), plus **★ Favorites**, **🕘 Recently viewed** (yours) and **🔥 Most viewed** (team, last 60 days). Tap ☆ on any document to add it to your shelf.
...
- **ISO document control (QP-DC-01 / QP-DC-02)** — a controlled-document workflow on top of the library:
  ...
  - **DAR → approval workflow** — submit a Document Action Request, then a single approval makes it the effective MASTER DOCUMENT (`Draft → Pending Approval → MASTER DOCUMENT`). An approval queue lets the approver approve or reject (back to Draft with a comment). Effective date and next-review date (+1 year) are recorded on final approval.
  ...
  - **Electronic approval record** — replaces the physical Approved/Checked/Prepared stamps: each document shows who prepared and approved it, with timestamps, recorded automatically from the authenticated user when they act in the approval queue.
  ...
  - **Batch DAR** — submit several documents on one Document Action Request: drop multiple files, each auto-classified from its document number, all created together (shared batch) and shown as one group in the approval queue with a single **Approve all**. Mirrors the real FDC-001, which lists many documents per request.
  - **Import a filled FDC-001 (Excel)** — attach a completed DAR form and the whole DAR is auto-filled (parsed with `xlsx`): the revision table (Doc No / Document Name / Page / Old Revise / New Revise / Effective Date) plus From / Date / Section / Comment, with Type and Request-for inferred from the document number and the revise columns. No double entry.
